...
const ERROR_MESSAGE =
  "internal server error. please refresh page and try again";
const DateInput = forwardRef(({ value, onClick }, ref) => (
  <Input
    onClick={onClick}
    placeholder={value ? value : "choose date"}
    ref={ref}
  />
));
...
function AddSpecialPriceForm(props) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      type: Yup.string()
        .matches(/^nominal|percentage$/)
        .required(),
      amount: Yup.number()
        .test({
          name: "min",
          test: function (value, context) {
            return context.parent.type === "nominal"
              ? value > 0
              : value >= -100 && value !== 0;
          },
  ...
  const fetchRoom = useCallback(async () => {
    ...
    const response = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}/room/tenant/get`,
      {
        params: {
          propertyId: selectedPropeties.map((property) => property.value),
        },
      }
    );
    ...
    setRoomOptions(
      response.data.result.map((room) => {
        return {
          value: room.id,
          label: `${room.Property.name} | ${room.name}`,
        };
      })
    );
  ...
  useEffect(() => {
    ...
    const validRooms = selectedRooms.filter((room) =>
      validRoomIds.includes(room.value)
    );
      ...
      <ModalBody>
          ...
          <Box w="50%" mr={2}>
            ...
            <DatePicker
              ...
              onChange={(date) => {
                ...
                if (new Date(formik.values.endDate) < new Date(date))
          ...
          <Box w="50%">
            ...
            <DatePicker
              ...
              onChange={(date) => {
                ...
                if (new Date(formik.values.startDate) > new Date(date))
        ...
        <FormControl my={5}>
          ...
          <Select2
            ...
            value={selectedRooms}
          />
          {formik.errors.selectedRooms ? (
            <Text color={"red"}>*{formik.errors.selectedRooms}</Text>
          ) : null}
        </FormControl>
        ...
        <FormControl my={5}>
          ...
          {formik.values.type !== "nominal" ? (
            ...
            >
              Percentage <strong>below 0 = Discount</strong>
              <br />
      ...
      <ModalBody>
        ...
        >
          <Image
            ...
            maxH={"150px"}
            src={
              props.specialPrice.Room.Property.pic
        ...
        <FormControl my={5}>
          ...
          <Select2
            ...
            defaultValue={
              formik.values.type === "nominal" ? types[0] : types[1]
            }
          />
          {formik.errors.type ? (
            <Text color={"red"}>*{formik.errors.type}</Text>
        ...
        <Table>
          <Tbody>
            ...
            <Tr>
              <Th>Duration</Th>
              <Td>
                {new Date(props.specialPrice.startDate).toLocaleDateString(
                  "id"
                )}{" "}
                -{" "}
                {new Date(props.specialPrice.endDate).toLocaleDateString("id")}
              </Td>
            </Tr>
            <Tr>
              <Th>Amount</Th>
              <Td>
                {props.specialPrice.type === "nominal" ? "Rp. " : null}
                {props.specialPrice.discount}
                {props.specialPrice.type !== "nominal" ? "%" : null}
              </Td>
            </Tr>
          </Tbody>
        </Table>
    ...
    <Box mt="20px">
      ...
      <Flex justifyContent={"space-between"} my={5}>
        ...
        <Button
          ...
          fontSize={"sm"}
          w={"0.3 vw"}
          leftIcon={<i className="fa-solid fa-filter" />}
      ...
      >
        <FilterSpecialPrice
          ...
          handleChangeStartDate={(item) =>
            setStartDate(item.target.valueAsDate)
          }
                  ...
                  >
                    ...
                    >
                      <Image
                        h={"100%"}
                        src={
                          specialPrice.Room.Property.pic
                            ? process.env.REACT_APP_API_BASE_URL +
                              specialPrice.Room.Property.pic
                            : "/Assets/add_photo.png"
                        }
                    ...
                    <Box>
                      <Text fontWeight="bold" fontSize={"sm"}>
                        {specialPrice.Room.Property.name} -{" "}
                        {specialPrice.Room.name}
                      </Text>
                      <Text fontWeight="reguler" fontSize={"xs"}>
                        Last Modified:{" "}
                        {new Date(specialPrice.updatedAt).toLocaleDateString()}
                      </Text>
                    </Box>
                  </Td>
                  <Td>
                    <Text fontWeight="reguler" fontSize={"xs"}>
                      {new Date(specialPrice.startDate).toLocaleDateString()} -{" "}
                      {new Date(specialPrice.endDate).toLocaleDateString()}
                    </Text>
                  </Td>
                  <Td>
                    <Text
                      ...
                      fontSize={"xs"}
                      textAlign={
                        specialPrice.type === "percentage" ? "center" : "left"
                      }
                    >
                      {specialPrice.type === "percentage"
                        ? specialPrice.discount + "%"
                        : "Rp. " +
                          specialPrice.discount.toLocaleString() +
                          ",00"}
                    </Text>
                  </Td>
                  <Td>
      ...
      <ReactPaginate
        previousLabel={
          <i className="fa-solid fa-chevron-left" style={{ fontSize: 18 }}></i>
        }
        nextLabel={
          <i
      ...
      <ModalBody>
        ...
        >
          <Image
            ...
            maxH={"150px"}
            src={
              props.roomUnavailability.Room.Property.pic
                ? process.env.REACT_APP_API_BASE_URL +
                  props.roomUnavailability.Room.Property.pic
                : "/Assets/add_photo.png"
            }
        ...
        <Table>
          <Tbody>
            ...
            <Tr>
              ...
              <Td>
                {new Date(
                  props.roomUnavailability.startDate
                ).toLocaleDateString("id")}{" "}
                -{" "}
                {new Date(props.roomUnavailability.endDate).toLocaleDateString(
                  "id"
                )}
              </Td>
            </Tr>
      ...
      <ModalFooter>
        <Button
          ...
          onClick={async () => {
            ...
            try {
              const result = await axios.post(url, {
                id: props.roomUnavailability.id,
              });
              props.fetchRoomUnavailability();
              props.disclosure.onClose();
            } catch (err) {
                ...
                <Tr key={`roomUnavalailability-${roomUnavailability.id}`}>
                  ...
                  >
                    ...
                    >
                      <Image
                        h={"100%"}
                        src={
                          roomUnavailability.Room.Property.pic
                            ? process.env.REACT_APP_API_BASE_URL +
                              roomUnavailability.Room.Property.pic
                            : "/Assets/add_photo.png"
                        }
                    ...
                    <Box>
                      <Text fontWeight="bold" fontSize="sm">
                        {roomUnavailability.Room.Property.name} -{" "}
                        {roomUnavailability.Room.name}
                      </Text>
                      <Text fontWeight="reguler" fontSize="xs">
                        Last Modified:{" "}
                        {
                          new Date(roomUnavailability.updatedAt)
                            .toISOString()
                            .split("T")[0]
                        }
                      </Text>
                  ...
                  <Td>
                    <Text fontSize="12px" fontWeight="reguler">
                      Starts :{" "}
                      {new Date(
                        roomUnavailability.startDate
                      ).toLocaleDateString("id")}
                      <br />
                      Ends&emsp;:{" "}
                      {new Date(roomUnavailability.endDate).toLocaleDateString(
                        "id"
                      )}
                    </Text>
                  </Td>
